...
%\fakesection{Logarithmic Functions} \label{logarithmic-intro}
\section{Logarithmic Functions Supplement} \label{appendix-logarithmic}

%Introduction

\subsection*{Examples} \label{examples-appendix-logarithmic}

\begin{myExample}~\\[-8mm]
\begin{minipage}{0.5\linewidth}
	The graph of $f(x)= \log_b(x)$ is given in Figure~\ref{fig:log-example1}.  \\Find the value of $b$.\\[3mm]
	Note that the points $(1,0)$ and $(9,2)$ are on the graph of $y=f(x)$.
\end{minipage}
\begin{minipage}{0.5\linewidth}
	\begin{center}
		\captionof{figure}{$y=\log_b(x)$}
		\begin{tikzpicture}
 			\begin{axis}[
 				framed,
	 			height=5cm,
				...
				\addplot[first,line width=1.5pt,<->]expression[domain=0.013:14,samples=400]{{ln(x)/ln(3)}};
				\addplot[first,line width=1.5pt,mark=*] coordinates {(1,0)};
				\addplot[first,line width=1.5pt,mark=*] coordinates {(9,2)};
			\end{axis}
 			\end{tikzpicture}
			\label{fig:log-example1}
	\end{center}
\end{minipage}
\end{myExample}
	...
	\begin{center}
		...
		\begin{tikzpicture}
 			\begin{axis}[
		 		...
		 		% use TeX as calculator:
				\addplot[first,line width=1.5pt,<->]expression[domain=0.018:28,samples=400]{{ln(x)/ln(5^(0.5))}};
				\addplot[first,line width=1.5pt,mark=*] coordinates {(1,0)};
				\addplot[first,line width=1.5pt,mark=*] coordinates {(25,4)};
			\end{axis}
 			\end{tikzpicture}
...
\begin{myExercise}
Each table represents a table of values for a function $f(x) = \log_b(x)$.  Find the value of $b$.
\begin{enumerate}
\begin{multicols}{2}
	\item ~\\[-8mm]
		\begin{center}
			\captionof{table}{$f(x)=\log_b(x)$}
